<?php
echo "Hello World2423432423434534dgfdfdf!";
session_start();
...
$sql = "SELECT * FROM educationprovider WHERE ";
...
if (!empty($_POST['edu-id']) && !empty($_POST['edu'])) {
	...
	$_SESSION['sqlsearchedu'] = $sql;
header("Location: search-education?recall=Y");
exit;
...
elseif(isset($_SESSION['sqlsearchedu']) && !empty($_SESSION['sqlsearchedu']) && $_GET['recall'] == "Y" ){$sql = $_SESSION['sqlsearchedu']; }
...
<title>Search Education Providers | iStudent</title>
...
<article>
		<div id="left2">
            ...
            <form name="search" action="/search-education" method="post">
                 <div class="form1">
                 		<label>State</label>
                        <select name="state">
            ...
            <input class="auto" type="text" id="edu" name="edu" value="" />
                 ...
                 <div class="form1">
                 		<label>Offering Scholarship</label>
                        <input type="radio" name="scholarship" value="Y" /> Y
                        <input type="radio" name="scholarship" value="" /> N
                 </div>
                 <div class="form1">
                 		<label>Insitute Type</label>
                        <select name="type">
                                <option value="">-- Please Select --</option>
                 ...
                 <div class="form1">
                 		<button class="orange-button width100">Search Now</button>
                                                <a href="/search-education"><button type="button" class="orange-button width100">Reset Search</button></a>
        ...
        <div id="right2">
		  ...
		  if(mysqli_num_rows($exec) >= 1){
			  echo "<h4>".mysqli_num_rows($exec)." Education Providers found</h4>";
			  

	$total_pages = mysqli_num_rows($exec);
	$targetpage = "search-education"; 	
	$limit = 15; 								
	$page = $_GET['page'];
		$adjacents = 3;
...
END;
			  ...
			  else{echo "<h4>0 Education Providers found</h4>";}
